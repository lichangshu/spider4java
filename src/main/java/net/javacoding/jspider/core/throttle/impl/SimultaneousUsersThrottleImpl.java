 ...
 * separately, and the latency between two requests is a random value between a
 * configurable minimum and maximum value. This way, a normal web user's
 * 'thinking time' can be faked. More inequal partitioning of requests on the
 * webserver will be the result, as with human users accessing a system.
 *
...
public class SimultaneousUsersThrottleImpl implements Throttle {
	 ...
	 *
	 * @param minThinkTime the minimum time between two requests from the same
	 * user
	 * @param maxThinkTime the maximum time between two requests from the same
	 * user
	 */
