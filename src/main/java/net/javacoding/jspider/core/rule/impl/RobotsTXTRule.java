 ...
 * file to the resources we want to fetch on that site. This file allows
 * webmasters to exclude certain resources and folders not to be spidered by web
 * robots, to disallow inclusion in search engines, etc ...
 *
...
public class RobotsTXTRule extends BaseRuleImpl {
	...
	/**
	 * all lines in the robots.txt file that apply to us and forbid access to a
	 * part of the site.
	 */
	...
	/**
	 * Returns the user agent from robots.txt we're obeying. (can be '*'). This
	 * user agent identification is the first match we encountered in the file,
